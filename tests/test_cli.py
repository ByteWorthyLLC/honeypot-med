...
class CliTest(unittest.TestCase):
    ...
    def test_share_bundle_outputs_html_json_and_markdown(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((Path(tmpdir) / "proof-dossier.html").exists())
            self.assertTrue((Path(tmpdir) / "offline-proof.pdf").exists())
            self.assertTrue((Path(tmpdir) / "ui-mockup.html").exists())
            ...
            self.assertIn("Open visual proof dossier", html)
            proof_html = (Path(tmpdir) / "proof-dossier.html").read_text(encoding="utf-8")
            self.assertIn("Offline proof dossier", proof_html)
            self.assertIn("Open PDF proof", proof_html)
            mockup_html = (Path(tmpdir) / "ui-mockup.html").read_text(encoding="utf-8")
            self.assertIn("Honeypot Med Studio Mockup", mockup_html)
            self.assertTrue((Path(tmpdir) / "offline-proof.pdf").read_bytes().startswith(b"%PDF-"))
            ...
            self.assertEqual(bundle["artifacts"]["proof_dossier_html"], "proof-dossier.html")
            self.assertEqual(bundle["artifacts"]["proof_dossier_pdf"], "offline-proof.pdf")
            self.assertEqual(bundle["artifacts"]["ui_mockup"], "ui-mockup.html")
    ...
    def test_lab_command_generates_offline_trap_artifacts(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            proof_html = (Path(tmpdir) / "proof-dossier.html").read_text(encoding="utf-8")
            self.assertIn("Visual dossier", proof_html)
            ui_mockup = (Path(tmpdir) / "ui-mockup.html").read_text(encoding="utf-8")
            self.assertIn("Proof console", ui_mockup)
            self.assertTrue((Path(tmpdir) / "offline-proof.pdf").read_bytes().startswith(b"%PDF-"))
